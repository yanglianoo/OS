...
//如果这个地址是数字，那么需要做相应的处理：指针指向下一个位置。
//减去'0'可以直接判断这个字符代表的数值。
//如果指针++后下一位数据也是数字，则该数字为超过10了。
//cpu识别内存的时候得按照小端来处理内存信息
//函数功能：将字符串转换为数字，例如 '356' 转换为 （int)356
...
static char *number(char *str,unsigned long num, int base,int size,int precision, int flags)
{
    ...
    //如果flas 指出是带符号数并且 num 小于0，则置符号变量 sign=负号，并使 num 取绝对值
    if(flags & SIGN && num < 0)
    {
        sign = '-';
        num = -num;
    ...
    i = 0;
    //如果数值 num 为0，则临时字符串='0';否则给定的基数将数值num转换为字符形式
    if(num == 0)
        tmp[i++] = '0';
    else
        while (num != 0)
    ...
    //若flags中没有左调整（左对齐）标志，则在剩余宽度中存放 c 字符（'0' 或空格）
    if(!(flags & LEFT))
        while (size-- > 0)
            *str++ = c;
    //此时 i 存有数值 num的数字个数
    //若数字小于精度值，则 str 中放入 （精度值 - i）个 '0'
    while (i < precision)
        *str++ = '0';
...
int vsprintf(char *buf, const char *fmt, va_list args)
{
    ...
    repeat:
        ...
        //取当前参数字段宽度阈值，放入 field_width 变量中
        field_width = -1;
